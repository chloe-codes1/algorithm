...
for t in range(1,T+1):
    ...
    code = ''
    for i in range(len(codes) -8):
        if codes[i: i+7] in data.keys():
            if codes[i+7: i+14] in data.keys():
                code = codes[i:i+56]
                break
    ...
    index = 0
    for _ in range(8):
        cryptogram.append(data.get(code[index:index+7]))
        index += 7


    result = 0

    if ((int(cryptogram[0]) + int(cryptogram[2]) + int(cryptogram[4])+ int(cryptogram[6])) *3 + int(cryptogram[1]) + int(cryptogram[3]) + int(cryptogram[5]) + int(cryptogram[7])) %10 ==0:
        cryptogram = list(map(int, cryptogram))
        result = sum(cryptogram)
