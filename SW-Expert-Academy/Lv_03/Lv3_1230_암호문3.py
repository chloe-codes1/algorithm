...
for t in range(1, T+1):
    ...
    for i in range(number):
        ...
        elif command[start] =='D':
            for _ in range( int(command[start +2])):
                cryptogram.pop( int(command[start +1]))
            start += 3
        elif command[start] == 'A':
            for i in range(int(command[start +1])):
                cryptogram.append( command[start +2 +i : start+3 +i])
            start += int(command[start +1]) +2
